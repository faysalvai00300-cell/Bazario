...
@section('title', 'Forgot Password - SmartLookBD')
...
<div class="fixed inset-0 flex flex-col items-center justify-center bg-white z-[99999] p-6 overflow-y-auto">
    ...
    <div class="w-full max-w-sm bg-[#FDECEC] rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-300 p-8 sm:p-10 flex flex-col auth-card-animate">
        ...
        <div x-data="{ 
            mode: '{{ old('reset_type', request('mode', $errors->has('phone') ? 'phone' : ($errors->has('email') ? 'email' : 'select'))) }}' 
            ...
            <div x-show="mode === 'select'" class="animate-fade-in text-center">
                ...
                <p class="text-gray-500 text-sm mb-10">Choose how you want to reset your password</p>

                <div class="space-y-4">
                    <button @click="mode = 'phone'" class="w-full flex items-center justify-center gap-3 bg-white border border-gray-200 p-4 rounded-xl font-bold text-gray-700 transition-all shadow-sm hover:border-blue-400 group h-[65px]">
                    ...
                    <button @click="mode = 'email'" class="w-full flex items-center justify-center gap-3 bg-white border border-gray-200 p-4 rounded-xl font-bold text-gray-700 transition-all shadow-sm hover:border-blue-400 group h-[65px]">
            ...
            <div x-show="mode !== 'select'" class="animate-fade-in">
                <div class="flex items-center gap-3 mb-8">
                    ...
                    <h1 class="text-2xl font-bold text-gray-900" x-text="mode === 'phone' ? 'Reset via Phone' : 'Reset via Email'"></h1>
                </div>

                @if (session('success'))
                    <div class="mb-6 p-4 bg-green-50 text-green-700 rounded-xl text-sm border border-green-100 font-bold">
                        {{ session('success') }}
                    </div>
                @endif
                ...
                <form action="{{ route('password.email') }}" method="POST" class="space-y-4">
                    @csrf
                    <input type="hidden" name="reset_type" :value="mode">
                    ...
                    <div x-show="mode === 'phone'">
                        ...
                        <div class="flex w-full bg-[#EBF2FF] rounded-xl overflow-hidden h-[58px] focus-within:border-blue-500 transition-all" style="border: 1px solid #94a3b8 !important;">
                            ...
                            <input type="tel" name="phone" value="{{ old('phone') }}" placeholder="1712345678" maxlength="11"
                    ...
                    <button type="submit" class="w-full py-4 rounded-xl shadow-xl hover:shadow-2xl transition-all active:scale-[0.98] mt-4 flex items-center justify-center" style="background-color: #000000 !important;">
                        <span class="font-bold text-lg" style="color: #ffffff !important;" x-text="mode === 'phone' ? 'Send Code' : 'Send Code'"></span>
                    </button>
                </form>
            </div>

            <div class="mt-8 text-center border-t border-gray-100 pt-6">
                <p class="text-gray-500 text-sm">Remembered your password? <a href="{{ route('login') }}" class="text-blue-500 font-bold hover:underline">Sign in now</a></p>
            </div>
        </div>
    <style>

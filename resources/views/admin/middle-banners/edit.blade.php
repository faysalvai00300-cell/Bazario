...
@section('title', 'Edit Middle Banner')
@section('content')

<div class="mb-6 flex items-center justify-between">
    <h2 class="text-xl font-bold text-gray-900 dark:text-white">Edit Middle Banner</h2>
...
<div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 max-w-3xl dark:bg-gray-800 dark:border-gray-700">
    <form action="{{ route('admin.middle-banners.update', $banner) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-5">
            <div>
                ...
                <input type="url" name="image_url" value="{{ old('image_url', $banner->image && Str::startsWith($banner->image, 'http') ? $banner->image : '') }}" 
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1 dark:text-gray-300">Banner Image</label>
                ...
                <!-- Image Preview Area -->
                <div id="image_preview_container" class="mt-3 {{ $banner->image ? '' : 'hidden' }}">
                    <img id="image_preview" src="{{ $banner->image_url }}" alt="Banner Preview" class="h-48 w-full object-cover rounded-2xl border-2 border-dashed border-[#FF6A00]/50 p-1.5 bg-orange-50/30 dark:bg-orange-900/10">
                </div>
            </div>
            ...
            <div class="pt-4 border-t border-gray-100 dark:border-gray-700 space-y-4">
                <div class="flex items-center gap-3">
                    ...
                    <input type="checkbox" name="show_on_desktop" id="show_on_desktop" value="1" {{ old('show_on_desktop', $banner->show_on_desktop) ? 'checked' : '' }} 
                ...
                <div class="flex items-center gap-3">
                    ...
                    <input type="checkbox" name="show_on_mobile" id="show_on_mobile" value="1" {{ old('show_on_mobile', $banner->show_on_mobile) ? 'checked' : '' }} 
                        class="w-5 h-5 rounded border-gray-300 text-orange-600 focus:ring-orange-500 dark:bg-gray-900 dark:border-gray-700">
        ...
        <div class="flex justify-end gap-3 mt-8">
            ...
            <button type="submit" class="admin-primary-btn px-6 py-2.5 rounded-xl font-bold transition-colors shadow-sm">Update Middle Banner</button>
        </div>
    </form>
</div>

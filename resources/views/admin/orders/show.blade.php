...
<div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
    <div class="w-full sm:w-auto">
        <h2 class="text-xl sm:text-2xl font-black text-gray-900 dark:text-white flex items-center gap-2 sm:gap-3 flex-wrap">
            <a href="{{ route('admin.orders.index') }}" class="p-2 hover:bg-gray-100 rounded-full transition dark:hover:bg-gray-700" title="Back to Orders">
                <i data-lucide="arrow-left" class="w-5 h-5 sm:w-6 h-6 text-gray-600 dark:text-gray-400"></i>
            </a>
            <span class="truncate">Order {{ $order->order_number }}</span>
            <span class="px-2 py-0.5 sm:px-3 sm:py-1 text-[10px] sm:text-sm rounded-full font-bold
        ...
        <p class="text-gray-500 text-[10px] sm:text-sm mt-1 dark:text-gray-400">Placed on {{ $order->created_at->format('F d, Y \a\t h:i A') }}</p>
    </div>
        ...
        <div class="flex items-center bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 p-0.5 rounded-none overflow-hidden shadow-sm">
            ...
            <div class="px-3 flex-1">
                ...
                <form action="{{ route('admin.orders.send-to-courier', $order) }}" method="POST" class="flex items-center gap-2">
                    @csrf
                    <select name="courier_type" class="text-xs sm:text-sm font-bold border-gray-200 py-2 pl-2 sm:pl-3 pr-8 sm:pr-10 focus:ring-green-500 rounded-none dark:bg-gray-900 dark:border-gray-700 dark:text-gray-300 h-[38px] sm:h-[42px] w-full sm:w-auto">
                        <option value="steadfast">Steadfast</option>
                        <option value="redx">RedX</option>
                        <option value="pathao">Pathao</option>
                    </select>
                    <button type="submit" style="background-color: #45b86f !important; color: white !important;" class="hover:bg-green-700 px-3 sm:px-5 py-2 rounded-none text-[10px] sm:text-sm font-black uppercase tracking-wider transition shadow-md active:scale-95 flex items-center gap-1 sm:gap-2 h-[38px] sm:h-[42px] whitespace-nowrap">
                        <i data-lucide="send" class="w-3.5 h-3.5 sm:w-4 h-4"></i> Dispatch
                    </button>
                </form>
                @else
                <div class="flex items-center gap-2 px-2 py-2 text-blue-600 bg-blue-50 dark:bg-blue-900/20">
                    <i data-lucide="truck" class="w-3.5 h-3.5 sm:w-4 h-4"></i>
                    <span class="text-[10px] sm:text-xs font-black uppercase tracking-tight">{{ $order->courier_name }}: {{ $order->courier_tracking_id }}</span>
                </div>
                @endif
            </div>
        </div>

        <form action="{{ route('admin.orders.status', $order) }}" method="POST" class="flex gap-2 w-full sm:w-auto">
            @csrf @method('PATCH')
            <select name="status" class="flex-1 border-gray-200 rounded-lg text-xs sm:text-sm focus:ring-orange-500 font-medium dark:bg-gray-900 dark:border-gray-700 dark:text-white h-[38px] sm:h-[42px] {{ $order->status === 'delivered' ? 'border-green-500' : '' }}">
                @if($order->status === 'delivered')
                    <option value="delivered" selected>Delivered (Locked)</option>
            ...
            <button type="submit" class="bg-[#FF6A00] hover:bg-[#FF7A1A] text-white px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-bold shadow-sm transition-all active:scale-95 whitespace-nowrap">Update</button>
        </form>
    </div>
</div>
    ...
    <div class="lg:col-span-2 space-y-6">
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 dark:bg-gray-800 dark:border-gray-700 transition-colors">
            ...
            <div class="space-y-4">
                ...
                <div class="flex flex-col sm:flex-row items-center sm:items-start gap-3 sm:gap-4 py-4 border-b border-gray-100 dark:border-gray-700 last:border-0 last:pb-0">
                    <img src="{{ $item->product ? $item->product->getColorImageUrl($item->color) : asset('placeholder.png') }}" 
                        alt="{{ $item->product_name }}" 
                        class="w-20 h-20 sm:w-24 sm:h-24 rounded-2xl object-cover bg-gray-50 dark:bg-gray-900 cursor-zoom-in hover:opacity-80 transition shadow-sm border border-gray-200 dark:border-gray-700"
                    ...
                    <div class="flex-1 min-w-0 w-full">
                        <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-2 sm:gap-4">
                            <div class="flex-1">
                                <h4 class="font-bold text-gray-900 truncate dark:text-white flex items-center gap-2 text-sm sm:text-base">
                                    {{ $item->product_name }}
                                    @if($item->product && $item->product->sku)
                                        <span class="text-[8px] sm:text-[10px] bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded font-black uppercase tracking-widest border border-slate-200 dark:bg-slate-800 dark:text-slate-400 dark:border-slate-700">
                                ...
                                <div class="flex flex-wrap items-center gap-2 mt-1">
                                    <form action="{{ route('admin.order-items.update', $item) }}" method="POST" class="flex items-center">
                                        @csrf @method('PATCH')
                                        <span class="text-[10px] sm:text-xs text-gray-500 mr-1 dark:text-gray-400">Qty:</span>
                                        <input type="number" name="quantity" value="{{ $item->quantity }}" min="1" 
                                            class="w-12 sm:w-16 h-7 sm:h-8 text-[10px] sm:text-xs font-bold border-gray-200 rounded-lg focus:ring-orange-500 py-1 px-2 dark:bg-gray-900 dark:border-gray-700 dark:text-white">
                                        <button type="submit" class="ml-1 px-2 py-1 bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white rounded-lg transition dark:bg-blue-900/20 dark:text-blue-400 flex items-center gap-1 border border-blue-100 dark:border-blue-800" title="Update Quantity">
                                            <i data-lucide="check" class="w-3 sm:w-3.5 h-3 sm:h-3.5"></i>
                                            <span class="text-[8px] sm:text-[9px] font-bold">Save</span>
                                        </button>
                                    </form>
                                    <span class="text-gray-300 dark:text-gray-600 mx-0.5 sm:mx-1">|</span>
                                    <span class="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400">Tk{{ number_format($item->price) }}</span>
                                </div>
                                <div class="flex flex-wrap gap-1.5 sm:gap-2 mt-2">
                                    @if($item->size)
                                    <span class="text-[8px] sm:text-[9px] bg-orange-50 text-orange-600 px-2 py-0.5 rounded font-black uppercase tracking-wider dark:bg-orange-900/30 dark:text-orange-400">Size: {{ $item->size }}</span>
                                    @endif
                                    @if($item->color)
                                    <span class="inline-flex items-center gap-1 sm:gap-1.5 text-[8px] sm:text-[9px] bg-blue-50 text-blue-600 px-2 py-0.5 rounded font-black uppercase tracking-wider dark:bg-blue-900/30 dark:text-blue-400">
                                        @if(str_starts_with($item->color, '#'))
                                            <span style="background-color: {{ $item->color }}" class="w-3 h-3 sm:w-5 h-5 rounded-md border border-gray-300 shadow-sm dark:border-gray-600"></span>
                                        @endif
                                        Color: {{ $item->color }}
                                    </span>
                                    @endif
                            ...
                            <div class="flex sm:flex-col justify-between sm:justify-start items-center sm:items-end gap-2 sm:gap-0 mt-2 sm:mt-0 pt-2 sm:pt-0 border-t sm:border-t-0 border-gray-50 dark:border-gray-700">
                                <div class="font-black text-gray-900 dark:text-white text-sm sm:text-base">Tk{{ number_format($item->total) }}</div>
                                <form action="{{ route('admin.order-items.remove', $item) }}" method="POST" onsubmit="return confirm('Remove this item from order?')">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="text-red-500 hover:text-red-700 p-1.5 sm:p-1 rounded-lg hover:bg-red-50 transition dark:hover:bg-red-900/20" title="Remove Item">

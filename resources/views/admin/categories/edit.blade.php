    ...
    <h2 class="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
        ...
        @if($category->target_page)
            <span class="text-sm px-3 py-1 bg-cyan-100 text-cyan-700 rounded-none dark:bg-cyan-900/30 dark:text-cyan-400 border border-cyan-200 dark:border-cyan-800">
                Category Page {{ $category->target_page }}
            </span>
        @endif
    </h2>
    <a href="{{ route('admin.categories.index', $category->target_page ? ['target_page' => $category->target_page] : []) }}" class="text-sm text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300 flex items-center gap-1 group">
...
<form action="{{ route('admin.categories.update', $category) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="meta_title" value="{{ old('meta_title', $category->meta_title) }}" placeholder="Meta Title"
                        class="w-full border border-gray-300 rounded-none px-4 py-2 text-sm focus:ring-1 focus:ring-orange-400 focus:outline-none dark:bg-gray-900 dark:border-gray-700 dark:text-white">
                </div>
                <div>
                    <label class="text-sm font-medium text-gray-700 mb-1.5 block dark:text-gray-300">Meta Keywords</label>
                    <input type="text" name="meta_keywords" value="{{ old('meta_keywords', $category->meta_keywords) }}" placeholder="Keywords"
                        class="w-full border border-gray-300 rounded-none px-4 py-2 text-sm focus:ring-1 focus:ring-orange-400 focus:outline-none dark:bg-gray-900 dark:border-gray-700 dark:text-white">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="meta_description" rows="2" placeholder="Brief description for SEO..."
                    class="w-full border border-gray-300 rounded-none px-4 py-2 text-sm focus:ring-1 focus:ring-orange-400 focus:outline-none resize-none dark:bg-gray-900 dark:border-gray-700 dark:text-white">{{ old('meta_description', $category->meta_description) }}</textarea>
            </div>
            ...
            <div>
                ...
                <select name="linked_category_id" class="w-full border border-gray-300 rounded-none px-4 py-2 text-sm focus:ring-1 focus:ring-orange-400 focus:outline-none dark:bg-gray-900 dark:border-gray-700 dark:text-white">
                    ...
                    @foreach($categories as $cat)
                        <option value="{{ $cat->id }}" {{ old('linked_category_id', $category->linked_category_id) == $cat->id ? 'selected' : '' }}>
                            {{ $cat->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="pt-4 border-t border-gray-200 dark:border-gray-700">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                        ...
                        @if($category->target_page)
                            <input type="hidden" name="target_page" id="targetPage" value="{{ $category->target_page }}">
                            <div class="w-full border border-cyan-200 bg-cyan-50/50 rounded-none px-4 py-2 text-sm font-bold text-cyan-700 flex items-center justify-between">
                                <span>Page {{ $category->target_page }}</span>
                    ...
                    <div>
                        ...
                        <div class="flex flex-wrap gap-2 mt-2">
                            @php 
                                $currentGenders = (array) old('target_gender', $category->target_gender ?? []);
                            @endphp
        ...
        <div class="space-y-4">
            <div>
                ...
                <textarea name="description" rows="5" placeholder="Detailed category description..."
                    class="w-full border border-gray-300 rounded-xl px-4 py-2 text-sm focus:ring-1 focus:ring-orange-400 focus:outline-none resize-none dark:bg-gray-900 dark:border-gray-700 dark:text-white">{{ old('description', $category->description) }}</textarea>
            </div>
                ...
                <div class="border-2 border-dashed border-gray-300 rounded-2xl p-4 bg-gray-50 dark:bg-gray-900/50 dark:border-gray-700">
                    <label for="cat-image" class="cursor-pointer group flex flex-col items-center justify-center gap-2">
                        <div class="w-full h-32 relative rounded-xl overflow-hidden bg-white dark:bg-gray-800 flex items-center justify-center border border-gray-200 dark:border-gray-700">
                            @if($category->image)
                                <img id="image-preview" src="{{ asset('storage/' . $category->image) }}" alt="" class="absolute inset-0 w-full h-full object-cover">
                            @else
                                <img id="image-preview" src="" alt="" class="hidden absolute inset-0 w-full h-full object-cover">
                            @endif
                            <div id="image-placeholder" class="{{ $category->image ? 'hidden' : '' }} text-center">
            ...
            <div class="{{ ($category->target_page != 2 && $category->target_page != 3 && $category->target_page != 5) ? 'hidden' : '' }}" id="viewAllImageContainer">
                ...
                <div class="border-2 border-dashed border-gray-300 rounded-2xl p-4 bg-gray-50 dark:bg-gray-900/50 dark:border-gray-700">
                    <label for="view-all-image" class="cursor-pointer group flex flex-col items-center justify-center gap-2">
                        <div class="w-full h-32 relative rounded-xl overflow-hidden bg-white dark:bg-gray-800 flex items-center justify-center border border-gray-200 dark:border-gray-700">
                            @if($category->view_all_image)
                                <img id="view-all-preview" src="{{ asset('storage/' . $category->view_all_image) }}" alt="" class="absolute inset-0 w-full h-full object-cover">
                            @else
                                <img id="view-all-preview" src="" alt="" class="hidden absolute inset-0 w-full h-full object-cover">
                            @endif
                            <div id="view-all-placeholder" class="{{ $category->view_all_image ? 'hidden' : '' }} text-center">
                                <i data-lucide="upload-cloud" class="w-8 h-8 text-gray-300 group-hover:text-orange-500 transition-colors mx-auto mb-2"></i>
                                <span class="text-xs text-gray-400">Click to upload (JPG, PNG, WebP)</span>
                                <p class="text-[10px] text-gray-400 mt-1">Max size: 20MB</p>
                            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="sort_order" value="{{ old('sort_order', $category->sort_order) }}" required
                        class="w-full border border-gray-300 rounded-none px-4 py-2 text-sm focus:ring-1 focus:ring-orange-400 focus:outline-none dark:bg-gray-900 dark:border-gray-700 dark:text-white">
                </div>
                <div class="flex items-end">
                    <label class="flex items-center gap-3 cursor-pointer p-2 px-4 bg-gray-50 rounded-none border border-gray-300 dark:bg-gray-900/50 dark:border-gray-700 w-full h-[40px]">
                        <input type="hidden" name="is_active" value="0">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $category->is_active) ? 'checked' : '' }} class="rounded-none text-orange-500 focus:ring-orange-500 w-5 h-5">
    ...
    <div class="mt-8 pt-6 border-t border-gray-200 dark:border-gray-700 flex justify-end">
        <button type="submit" class="bg-[#FF6A00] hover:bg-[#FF7A1A] text-white px-12 py-3 rounded-none font-bold transition-all shadow-lg shadow-orange-500/20 flex items-center gap-2 transform active:scale-95 text-sm">
            <i data-lucide="save" class="w-5 h-5"></i> UPDATE CATEGORY
        </button>
    </div>
</div>
</form>
...
<script>
    ...
    function updateBoxOptions() {
        ...
        const currentBox = "{{ old('target_box', $category->target_box ?? '') }}";
        ...
        if (page && boxCounts[page]) {
            ...
            for (let i = 1; i <= boxCounts[page]; i++) {
                ...
                if (pageOccupied[i]) {
                    if (pageOccupied[i] === currentCategoryName && currentBox == i) {
                        label += ` (Current)`;
                    } else {
                        label += ` (Occupied: ${pageOccupied[i]})`;
                        option.style.color = '#94a3b8'; // gray-400
                    }
                }

...
<div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 w-full mx-auto dark:bg-gray-800 dark:border-gray-700">
    <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6"
        x-data="{ 
            targetPageFilter: '{{ request('target_page', '') }}',
            ...
            columns: ['Size', 'Chest (round)', 'Length'],
            rows: [['M', '39', '27.5']],
            ...
            colorMethod: 'manual',
            is_mega_deal: {{ old('is_mega_deal', false) ? 'true' : 'false' }}
        }">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="space-y-5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select id="target_page_filter" x-model="targetPageFilter" class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-orange-400 focus:outline-none dark:bg-gray-900 dark:border-gray-700 dark:text-white dark:focus:ring-orange-500/50">
                            ...
                            <option value="1">Category Page 1</option>
                            <option value="2">Category Page 2</option>
                            <option value="3">Category Page 3</option>
                            <option value="4">Category Page 4</option>
                            <option value="5">Category Page 5</option>
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="category_id" id="category_select" required x-model="selectedCategoryId"
                            ...
                            <option value="{{ $cat->id }}" data-page="{{ $cat->target_page }}" {{ old('category_id') == $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                            @endforeach
                        </select>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="number" name="buying_price" value="{{ old('buying_price') }}" required min="0" step="0.01"
                            class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-orange-400 focus:outline-none dark:bg-gray-900 dark:border-gray-700 dark:text-white dark:focus:ring-orange-500/50">
                    </div>
                    <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="text" name="sku" value="{{ old('sku') }}" placeholder="e.g. SLB-001"
                            class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-orange-400 focus:outline-none dark:bg-gray-900 dark:border-gray-700 dark:text-white dark:focus:ring-orange-500/50">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="sizes" value="{{ old('sizes') }}" placeholder="e.g. S, M, L, XL"
                            class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-orange-400 focus:outline-none dark:bg-gray-900 dark:border-gray-700 dark:text-white dark:focus:ring-orange-500/50">
                        ...
                        <div class="mt-3 p-3 border border-gray-200 rounded-xl dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/20">
                            <label class="flex items-center gap-3 cursor-pointer">
                                ...
                                <input type="checkbox" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="rounded text-blue-600 focus:ring-blue-600 w-5 h-5 dark:bg-gray-900 dark:border-gray-700">
                                <div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div x-data="{ addMethod: 'manual' }" class="space-y-4">
                            <div class="flex gap-2 p-1 bg-gray-100 dark:bg-gray-900 rounded-xl w-fit">
                                <button type="button" @click="addMethod = 'manual'" :class="addMethod === 'manual' ? 'bg-white dark:bg-gray-800 shadow-sm text-orange-600' : 'text-gray-500'" class="px-3 py-1.5 text-[10px] font-bold uppercase rounded-lg transition">Manual / RGB</button>
                                <button type="button" @click="addMethod = 'picker'" :class="addMethod === 'picker' ? 'bg-white dark:bg-gray-800 shadow-sm text-orange-600' : 'text-gray-500'" class="px-3 py-1.5 text-[10px] font-bold uppercase rounded-lg transition">Picker</button>
                                <button type="button" @click="addMethod = 'detect'" :class="addMethod === 'detect' ? 'bg-white dark:bg-gray-800 shadow-sm text-orange-600' : 'text-gray-500'" class="px-3 py-1.5 text-[10px] font-bold uppercase rounded-lg transition">Extract</button>
                            </div>
                ...
                <div>
                    ...
                    <div class="ck-editor-container">
                        <textarea name="description" id="editor" rows="5"
                            class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-orange-400 focus:outline-none resize-none dark:bg-gray-900 dark:border-gray-700 dark:text-white dark:focus:ring-orange-500/50">{{ old('description') }}</textarea>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="url" name="thumbnail_url" value="{{ old('thumbnail_url') }}" placeholder="https://example.com/image.jpg"
                        ...
                        <div>
                            ...
                            <input type="file" name="thumbnail" accept="image/*" id="thumbnail_input" required
                            ...
                            <!-- Image Preview Area -->
                            <div id="thumbnail_preview_container" class="mt-3 hidden relative w-32 h-32">
                                <img id="thumbnail_preview" src="" alt="Thumbnail Preview" class="h-32 w-32 object-cover rounded-xl border border-gray-200 dark:border-gray-700">
                                <a id="thumbnail_download_btn" href="" download class="absolute top-1 right-1 bg-blue-500 text-white rounded-full p-1.5 shadow-md hover:bg-blue-600 transition z-20" title="Download Image">
                        ...
                        <div class="pt-2">
                            <label class="text-sm font-medium text-gray-700 mb-2 block dark:text-gray-300">Gallery Images (Multiple)</label>
                            ...
                            <div id="gallery_preview_container" class="mt-3 grid grid-cols-3 sm:grid-cols-4 gap-3 hidden">
                ...
                <div class="space-y-5 lg:col-span-2">
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <label class="flex gap-3 p-4 border border-gray-200 rounded-xl cursor-pointer hover:bg-gray-50 transition items-start dark:border-gray-700 dark:hover:bg-gray-700/50">
                            ...
                            <input type="checkbox" name="is_top_sell" value="1" {{ old('is_top_sell') ? 'checked' : '' }} class="rounded text-orange-500 focus:ring-orange-500 w-5 h-5 mt-0.5 dark:bg-gray-900 dark:border-gray-700">
                            <div>
                            ...
                            <div>
                                ...
                                <div class="mt-2" x-show="is_mega_deal" x-cloak>
                                    ...
                                    <select name="mega_deal_effect" class="w-full border border-gray-200 rounded-lg px-2 py-1 text-[11px] focus:ring-1 focus:ring-orange-400 dark:bg-gray-900 dark:border-gray-700 dark:text-white">
                                        <option value="none" {{ old('mega_deal_effect') == 'none' ? 'selected' : '' }}>No Animation</option>
                                        <option value="shine" {{ old('mega_deal_effect') == 'shine' ? 'selected' : '' }}>Basic Shine</option>
                                        <option value="border" {{ old('mega_deal_effect') == 'border' ? 'selected' : '' }}>Rotating Border</option>
                                        <option value="confetti" {{ old('mega_deal_effect') == 'confetti' ? 'selected' : '' }}>Celebration (Confetti)</option>
                                        <option value="shine_border" {{ old('mega_deal_effect') == 'shine_border' ? 'selected' : '' }}>Shine + Border</option>
                                        <option value="all" {{ old('mega_deal_effect', 'all') == 'all' ? 'selected' : '' }}>Full Combo (All)</option>
                                    </select>
                                </div>
                            </div>
                        </label>
                        <label class="flex gap-3 p-4 border border-gray-200 rounded-xl cursor-pointer hover:bg-gray-50 transition items-start dark:border-gray-700 dark:hover:bg-gray-700/50">
                            ...
                            <input type="checkbox" name="is_new" value="1" {{ old('is_new') ? 'checked' : '' }} class="rounded text-orange-500 focus:ring-orange-500 w-5 h-5 mt-0.5 dark:bg-gray-900 dark:border-gray-700">
                            <div>
                        ...
                        <label class="flex gap-3 p-4 border border-gray-200 rounded-xl cursor-pointer hover:bg-gray-50 transition items-start dark:border-gray-700 dark:hover:bg-gray-700/50">
                            ...
                            <input type="checkbox" name="free_shipping" value="1" {{ old('free_shipping') ? 'checked' : '' }} class="rounded text-green-500 focus:ring-green-500 w-5 h-5 mt-0.5 dark:bg-gray-900 dark:border-gray-700">
                            <div>
                    ...
                    <div class="p-6 border border-gray-200 rounded-xl dark:border-gray-700 bg-gray-50/30 dark:bg-gray-900/10">
                        <div class="flex items-center justify-between mb-4">
                            <div class="flex items-center gap-4">
                                ...
                                <label class="relative inline-flex items-center cursor-pointer ml-4" x-data="{ active: true }">
            ...
            <div class="pt-6 border-t border-gray-100 flex flex-col sm:flex-row justify-end gap-3 mt-8 dark:border-gray-700">
                <a href="{{ route('admin.products.index', ['category_id' => request('category_id'), 'target_page' => request('target_page')]) }}" class="px-6 py-2.5 rounded-xl bg-gray-100 text-gray-700 font-bold hover:bg-gray-200 transition dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600 text-center sm:w-auto w-full order-2 sm:order-1">Cancel</a>
                <button type="submit" class="bg-[#FF6A00] hover:bg-[#FF7A1A] text-white px-8 py-2.5 rounded-xl text-sm font-bold shadow-lg shadow-orange-500/20 transition transform active:scale-95 sm:w-auto w-full order-1 sm:order-2">Save Product</button>
            </div>
        </div>
    </form>
</div>
...
<script>
    ...
    document.getElementById('thumbnail_input').addEventListener('change', function(event) {
        ...
        if (file) {
            ...
            reader.onload = function(e) {
                ...
                document.getElementById('thumbnail_download_btn').href = e.target.result;
                previewContainer.classList.remove('hidden');
            }
            reader.readAsDataURL(file);
        } else {
            previewImage.src = '';
            document.getElementById('thumbnail_download_btn').href = '';
    ...
    function renderGalleryPreviews() {
        ...
        selectedGalleryFiles.forEach((file, index) => {
            ...
            downloadBtn.className = 'bg-blue-500 hover:bg-blue-600 text-white rounded-full p-1 transition';
            downloadBtn.title = 'Download Image';
            downloadBtn.innerHTML = '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a2 2 0 002 2h12a2 2 0 002-2v-1M7 10l5 5m0 0l5-5m-5 5V3"/></svg>';
    ...
    // Form Submit Size Validation
    const form = document.querySelector('form');
    form.addEventListener('submit', function(e) {
        const thumbnailFile = document.getElementById('thumbnail_input').files[0];
        let totalSize = 0;
        ...
        // Current post_max_size is 8M. We check against 7.5MB to be safe.
        // Once php.ini is updated, this limit can be increased or removed.
        const maxSizeBytes = 100 * 1024 * 1024; // 100MB limit for post data

        if (totalSize > maxSizeBytes) {
            e.preventDefault();
            alert('মোট ফাইলের সাইজ খুব বেশি (প্রায় ' + (totalSize / (1024 * 1024)).toFixed(2) + ' MB)। \nদয়া করে কম ছবি দিন অথবা হাই-রেজোলিউশন ইমেজ কমিয়ে দিন। \nঅথবা সার্ভারের php.ini এ post_max_size বৃদ্ধি করুন।');
        }
    });
    ...
    function filterCategories() {
        ...
        const currentSelectedId = '{{ request('category_id', '') }}';
        ...
        originalOptions.forEach(option => {
            if (!selectedPage || option.value === "" || option.getAttribute('data-page') === selectedPage) {
                ...
                // Preserve selection if it matches currentSelectedId
    ...
    pageFilter.addEventListener('change', () => {
        filterCategories();
        // Reset selected category page when page filter changes
    ...
    function renderColorList() {
        ...
        productColors.forEach((item, index) => {
                ...
                <div class="flex items-center justify-between sm:justify-end gap-4 w-full sm:w-auto">
                    ...
                    <div class="relative group">
                        ...
                        <label for="swatch_file_${index}" class="cursor-pointer bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 p-1.5 rounded-lg hover:bg-gray-100 transition block">
                            <img id="swatch_preview_${index}" src="" class="w-5 h-5 object-cover rounded hidden">
                            <i data-lucide="image-plus" class="w-3.5 h-3.5 text-gray-400" id="swatch_icon_${index}"></i>
                        </label>
                    </div>
            ...
            // Add hidden inputs for the main colors array
